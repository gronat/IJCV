...
We evaluate the proposed per-location classifier learning approach on two different image descriptors: the bag-of-visual-words model (section~\ref{sec:bow_results} ) and Fisher vectors (section~\ref{sec:fisher_restuls}).  Next, we evaluate the recognition accuracy vs. the memory requirements (section~\ref{sec:memory_results}), and finally report qualitative results  (section~\ref{sec:qualitative_results}).   
...
%%%%%%%%%%%%%%%%%
  ...
  Results for the Fisher vector descriptor of different dimension $d\in{128, 512 and 2048}$ are shown in table~\ref{tab:recall01} and figure~\ref{fig:recall}.
 ...
 We report results only for the w-norm calibration as we found that the p-val calibration did not perform well together with the learnt Fisher vector representation.
We believe this may be attributed to the dimensionality of the representation, where the statistic of the classifier scores for the extremely high-dimensional
bag-of-visual-words TOFINISH.. 
