...
%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  This section (and almost the whole paper) implicitly speaks about tf-idf. However, there will be a section about FV later. We should re-design this section to be more general. We can keep \emph{Dual representaton} outside as separate section related to tf-idf.
   ...
   We are given tf-idf vectors $x_j$, one for each database image $j$. The goal is to learn a score $f_j$ for each database image $j$, so that, at test time, given the descriptor $q$ of the query image, we can either retrieve the correct target image as the image $j^*$ with the highest score 
   \begin{equation}
   \label{eq:class}
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      \noindent
      where $w_j$ is a weight vector re-weighting contributions of individual visual words and $b_j$ is the bias specific for image $j$. Given the training sets $\mathcal P_j$ and $\mathcal N_j$, the aim is to find a vector $w_j$ and bias $b_j$ such that the score difference between $x_j$ and the closest neighbor from its negative set $\mathcal N_j$ is {\em maximized}.
      \textcolor{red}{
      ...
      In our case (details in section~\ref{sec:experiments}), we use about 1-5 positive examples 
      \textcolor{red}{
        \emph{(Above we claim that only a single positive is available)}
      }
      , and 200 negative examples. As the dimensionality of $w$ is $100,000$ all training data points are typically support vectors. 
   \textcolor{red}{
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
      For each image $j$ in the database the linear SVM \eqref{eq:linear} is learned. The image is then represented by corresponding weight $w_j$ and bias $b_j$. The main disadvantage of this representation is that while the original high dimensional tf-idf vector $x_j$ is sparse the learned weight vector $w_j$ is dense. Hence a memory requirement for storage of the new representation is enormous.

      This drawback can be efficiently solved by storing SVM vectors in dual representation. The key observation is that the number of training data $K$ is significantly lower then dimensionality $D$ of the original tfidf vector. In the following we omit index $j$ for clarity. Since $K\ll D$ all the training data points are support vectors, hence each vector $w$ can be then expressed in dual form as lienar combination

      \begin{equation}
        w=\sum_{i\in \mathcal{P},\mathcal{N}} \alpha_i d_i = \mathcal{X} \cdot \vec{\alpha}
        \label{eq:dual}
      \end{equation}

      \noindent
      where $\alpha_i$, elemnts of vector $\vec{\alpha}$, are coefficients of linear combination of training data for $x_i$ and the matrix $\mathcal{X}$ contains descriptors of all database. Notice that vector $\vec{\alpha}$ is sparse and number of non-zero elements depends on the number of training data $K$. Finally, a column wise matrix $\mathcal{W}$ containing all learned weights can be expressed in dual form as
      ...
      \noindent
      where $\mathcal{A}$ is a sparse cofficient matrix where each column corresponds to vector $\vec{\alpha}$ from equation \eqref{eq:dual}. Hence, to be able to reconstruct the matrix $\mathcal{W}$ one has to keep only two sparse matrixes, the descriptor matrix $\mathcal{X}$ and coefficient matrix $\mathcal{A}$.
