...
%%%%%%%%%%%%%%%%%%%%%%%
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%
  \textcolor{petr}{
    The experiments are performed on two different challenging datasets, namely on the Pittsburgh dataset~\cite{Gronat13} contain and the Tokyo 24/7 dataset~\cite{Torii2015}. The details for each are provided next.
  }
    \paragraph{Pittsburgh dataset.}
      \textcolor{petr}{
      First, we performed experiments on a database of Google Streetview images from the Internet. We downloaded panoramas from Pittsburgh (U.S.) covering roughly an area of $1.3 \times 1.2 \; {\rm km}^2$. Similar to~\cite{Chen11}, we generate for each panorama 12 overlapping perspective views corresponding to two different elevation angles $4^\circ$ and $28^\circ$ to capture both the street-level scene and the building fa\c{c}ades. This results in a total of 24 perspective views each with $90^\circ$ FOV and resolution of $960 \times 720$ pixels. In this manner we generate two datasets. The first smaller dataset covers a smaller area and contains $25$k perspective images.  The second larger dataset contains $55$k images.
    }
    \textcolor{petr}{
      As a query set with known ground truth GPS positions, we use 8999 panoramas from the Google Streetview research dataset~\cite{GoogleResearch}, which cover approximately the same area, but were captured at a different time, and typically depict the same places from different viewpoints and under different illumination conditions.          
      We generate a test query set such that we first select a panorama at random, and second, we generate a perspective image with a random orientation and random elevation pitch. This way we synthesize 4,000 query test images. The Pittsburgh dataset is available at~\cite{GronatProjectPage}.
    }

    \paragraph{24/7 Tokyo dataset.}
    \textcolor{petr}{
      Similarly to the dataset above the 24/7 Tokyo dataset, available at project page of~\cite{Torii2015}, consists of a set of perspective images extracted from Google Streetview. It's $76k$ views covers roughly an area of $1.6 \times 1.6 \; {\rm km}^2$ of the city of Tokyo. The challenging query set has been collected by several appliances such as Apple-iPhone5s and Sony-Xperia smartphones. The query images have been captured at $125$ locations at different time of a day including night. The query set consist of $315$ query images.
    }
  ...
  %%%%%%%%%%%%%%%%%
    ...
    \begin{table}[tbp]
      \begin{centering}
        ...
        \caption{ 
          \textcolor{petr}{
            \textbf{Evaluation of the learnt Fisher vector representation on the Pittsburgh~\cite{Gronat13} and 24/7~Tokyo dataset~\cite{Torii2015}.}
    ...
    Results \textcolor{petr}{on Pirrsburght 25k} dataset of the proposed per-location learning method for the Fisher vector image representation for different dimensions are shown in table~\ref{tab:recallFV} and figure~\ref{fig:recall}.
...
\textcolor{petr}{
    At query time the calibrated e-SVM score~\eqref{eq:linear} must be computed. In case of w-norm method the calibration can be explicitly stored in a weight matrix. For p-val calibration method, each e-SVM score must be calibrated using $K$ stored values of the non-parametric CDF model. This requires to search for two closest values an interpolate between them \eqref{eq:empiricalCDF} which yields complexity of $O(N\log{K})$. Since $K$ is a constant both methods have linear time complexity at query time with different constant.
}
