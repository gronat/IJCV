...
\begin{abstract}
  ...
  \textcolor{petr}{  
    Experiments are performed on three datasets: 25,000 and 55,000 geotagged street view images of Pittsburgh, and the
    % new challenging 
    24/7 Tokyo benchmark containing $76,000$ images with varying illumination conditions. The results show improved place recognition accuracy of the learnt image representation over direct matching of raw image descriptors.
  }   
...
%
  ...
  \label{sec:failures}
    \textcolor{petr}{
      We have examined failure cases of the w-norm method w.r.t. the Fisher vector baseline on the Pittsburght 25k datset. We analyzed cases for which the w-norm method increases the rank of the first true positive by the baseline and for which this rank is decreased. Considering a shortlist  of the size $20$ we want to identify two cases: (i) an image with the rank of $\leq 20$ is pushed out of the short list using our method and (ii) an image with the rank of $\> 20$ is attracted into the short list.
    }


    \textcolor{petr}{
      We observe that in $43$ cases the rank of an image increased from $\leq 20$ typically to $20-40$. On the other hand, in $80$ cases a highly ranked images by the baseline ($40-70$) were attracted into the short list by the w-norm method. Finally, we observe that aggravation typically occurs on hard examples where the baseline rank is already high.
    }

    
    \textcolor{petr}{
      When visually inspecting the failure cases we observed that our method typically fails on queries containing a big portion of the sky clouds or vegetation, narrow streets or tunnels and sometimes retrieves images capturing the same building from different view point and from larger distances.
    }
