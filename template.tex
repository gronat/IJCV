...
%      but has not yet been used in conjunction with SVMs as done in this work.
  ...
  %Second, we note that the learnt $\vec{w}$ is not normalized. However, to measure descriptor similarity by a normalized dot-product,   
      ...
      \subsubsection{The need for normalization of $\vec{w}$} 
         Above we have shown that the learnt hyperplane $\vec{w}$ declines from the positive example $\vec{x}^+$ in such a way that loss on negative data is decreased. The aim is to use this learnt vector $\vec{w}$ as a new descriptor replacing the original positive data $\vec{x}^+$. However, we wish to measure the cosine of angle between the the new descriptor and the query image. This is equivalent to the normalized dot product, hence the vector $\vec{w}$ need to be normalized.
