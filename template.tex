...
%      but has not yet been used in conjunction with SVMs as done in this work.
  ...
  In Appendix we examine the per-exemplar SVM cost and give intuition why calibration by re-normalization works.
...
\bibliography{shortstrings,vggroup,cvww_template,mybib}
  ...
  \section*{Appendix} %\vspace{-0.2cm} 
    ...
    % }
    In section~\ref{sec:results} we show that the simple calibration by normalization often results in surprisingly good place recognition performance without the need for any additional positive or negative calibration data. In this section, we give a possible explanation why this simple calibration works.  We focus on the case of a single positive training example, i.e. when training set $\mathcal P = \vec{x}^{+}$, which is the typical case for place recognition where only one or at most few positive examples are available for each place.   In particular, we first analyze (in Section~\ref{sec:objective_analysis})  the SVM objective and show that the learnt hyperplane $\vec{w}$ can be interpreted as a new descriptor $\vec{x}^{*}$ that replaces the original positive example $\vec{x}^{+}$ and is re-weighted to increase its separation from the negative data. 
    Second, we show (in Section~\ref{sec:renormalization}) that when $\vec{x}^{*} = \frac{\vec{w}}{||\vec{w}||}$ is normalized, the dot-product $\vec{q}^T\vec{x}^*$ corresponds to measuring the cosine of the angle between the (normalized) query descriptor $\vec{q}$  and the new descriptor $\vec{x}^{*}$, which was found to work well in the literature for descriptor matching, as discussed in section~\ref{sec:calibration_by_renormalization}. The two steps are given next.  
