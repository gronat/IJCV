...
\paragraph{Linear Discriminant Analysis.}
  \textcolor{petr}{
    The linear discriminant analysis (LDA) has been recently used in~\cite{Aubry14} for image to 3D model alignment. Motivated by~\cite{Shrivastava11} the authors aim on detecting discriminative visual elements for painting alignment using e-SVM. They used a square euclidean loss instead of a hinge loss and drop the regularizer term in the e-SVM objective. It was shown that the optimal solution for this new optimization task can be found in a closed form and is equivalent to LDA. The covariance matrix of the data must be computed which makes this method unfeasible for high-dimensional data such as bag-of-words. Finally, similarly to e-SVM, authors note that calibration of the matching scores is also important.
  }
...
\textcolor{petr}{
  ...
  TODO: PCA whitening, LDA, CNN FC7?
}
%%%%%%%%%%%%%%%%%%%%%
\section{Conclusions}
%%%%%%%%%%%%%%%%%%%%%
