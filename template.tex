...
%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
      For each image $j$ in the database the linear SVM \eqref{eq:linear} is learned. The image is then represented by corresponding weight $\vec{w_j}$ and bias $b_j$. The main disadvantage of this representation is that when the original vector $\vec{x_j}$ is high dimensional and sparse, for instance tf-idf vector, the learned weight vector $\vec{w_j}$ is dense. Hence a memory requirement for storage of the new representation is enormous.

      This drawback can be efficiently solved by storing SVM vectors in dual representation. The key observation is that a number of training data $K$ is significantly lower then dimensionality $D$ of the original tfidf vector. In the following we omit index $j$ for clarity. Since $K\ll D$ all the training data points are support vectors, hence each vector $w$ can be then expressed in dual form as lienar combination
      ...
      \noindent
      where $\mathcal{A}$ is a sparse cofficient matrix where each column corresponds to vector $\vec{\alpha}$ from equation \eqref{eq:dual}. Hence, to be able to reconstruct the matrix $\mathcal{W}$ one has to keep only two sparse matrixes, the descriptor matrix $\mathcal{X}$ and the coefficient matrix $\mathcal{A}$.

      At query time, given the query image $q$, the uncalibrated SVM score between the query image and each of the database images can be computed by equation \eqref{eq:linear} which can be expressed in a matrix form as follows
      \begin{equation}
      ...
      \noindent
      It is worth noting that the new score $s$ is a linear combination of cosine similarity scores plus the bias term.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%  
  The calibration method described above requires to store a non-parametric model of each calibration function of learned SVM. Later, the SVM score calibration must be performed online at query time. These two aspects increase a memory footprint and computational cost. If $K$ values ot the empirical cumulative density function is kept for each database image, the additional memory footprint of $O(NK)$ is required to store all calibration functions for a dabase of size $N$. At query time, one has to find nearest values of the calibration function and interpolate between them \eqref{eq:interp} which yields a computational complexity of $O(N\;log(K))$. To construct a calibration function at offline phase the SVM scores just be sorted for each database image in turn. A complexity $O(N^2 log(N))$ makes this method hard to scale up.

  In this section we first simplify the calibration by approximating the cumulative density function by an affine line and show that no additional memory nor computationla cost is required at query time. Second, we address the offline stage problem by introducing a calibration by re-normalization and shift which reduces the offline comlexity to $O(N)$.

   \subsection{Affine approximation}
     Clearly the main disadvantage is the memory footprint and need for additional computation online. This can be eliminated simplification of the model such that each calibration function is represented as an affine line such that each calibration function
     \begin{equation}
       f_j(\vec{q}) = c_j\cdot s_j(\vec{q}) + d_j
       \label{eq:affiine}
     \end{equation}

     \noindent
     Substituting for $s_j(\vec{q})$ from equation \eqref{eq:svm} into equation above we get
     \begin{align}
     ...
     \noindent
     where $\vec{\tilde{q}_j}=[\vec{q}_j^T 1]^T$ and $\vec{\widetilde{w}}_j = [\vec{w}_j^T \beta_j]^T$. It is worthnoting that the vector $\vec{\tilde{w}_j}$  consisting of scaled SVM weight pus one coordinate can be computed offline. Therefore no additonal memory storage or computational cost is required at query time. The paremeters $c_j$, $d_j$ represent an approximation the calibration function. A selection of these parameters requires an expensive sorting of SVM scores for each database image in turn which yields a complexity of $O(N^2 \log(N))$ at offiline phase.

   \subsection{Calibration by re-scaling}
      Each SVM score can be calibrated such that first the bias term is substracted and second the score is scaled by inverse of the SVM weight vector. The calibraiton function can be then written as follows
      \begin{equation}
         f_j(\vec{q}) = \dfrac{1}{||\vec{w}_j||}\vec{q}^T\vec{w}_j
         \label{eq:rescale}
      \end{equation}

      This sort of calibration can be viewed as a spacial case of affine calibration where calibration parameters rely on SVM only. The equation \eqref{eq:rescale} can be further re-written such that
      ...
      \noindent
      where $\vec{\tilde{w}}_j=\frac{\vec{\tilde{w}}_j}{||\vec{w}_j||}$ can be computed offline or online and requires only one additional computation.
   \subsection{Discussion}
      The proposed calibration method works when the SVM is properelly regularized. The resulting direction of $\vec{\tilde{w}}$ havily depends on $C_2$ parameter of the objective \eqref{eq:obj}. It can be shown that when $C_2 \rightarrow 0$ the resulting $\vec{\tilde{w_j}} \rightarrow \vec{x}_j^+$ where $\vec{x}_j^+$ is a single positive exemplar used for training. Though this calibration is simple we observe that this type of calibration is more suited for dense descriptors such as Fisher vectors. Details will be given next in the experimental section. 
